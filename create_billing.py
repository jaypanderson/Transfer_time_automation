...
def open_file(option: int) -> str:
    ...
    """
    if option == 1:
        title = '①★追加預かり料金原本★★作成シート★★を選択してください。'
    elif option == 2:
...
def new_file_path(path: str, added_text: str = 'result') -> str:
    ...
    is provided the default 'result' will be used. (updated: Changed code so that it looks for the last occurrence of
    '.' so find the location to add the extra text for new file name. There was a bug where if a folder name had '.' in
     it, there would be a path error.)
...
def insert_name_date(sheet: Worksheet, year: int, month: int, class_name: str, child_name: str) -> None:
    ...
    :param child_name: The name of the child with extra charges. The child's name is represented as the first name, then
    a japanese space(/u3000) and then the first name. ex: '田中　太郎'.  '&' represents the last name and '$' represents
    the first name.
    :return: None
    """
    ...
    if len(full_name) != 2:
        messagebox.showinfo("スペースエラー", f"{full_name[0] if full_name else ''}さんの名前にスペースがない可能性があります。\n追加預かり料金原本と追加預かり料金原本★★作成シート★★を\n修正して、やり直してください。")
    last = full_name[0]
    first = full_name[1]
